do $$ begin
  alter type public.app_role add value 'reporter';
exception when duplicate_object then null;
end $$;
do $$ begin
  alter type public.app_role add value 'sub_editor';
exception when duplicate_object then null;
end $$;
do $$ begin
  alter type public.app_role add value 'news_editor';
exception when duplicate_object then null;
end $$;
do $$ begin
...
as $$
  select exists (
    ...
    where user_id = _user_id
      and role in ('admin', 'editor', 'news_editor', 'sub_editor', 'reporter')
  )
$$;
