...
export function DeskSocialPanel({ storyId }: { storyId: string }) {
  ...
  if (social.isLoading) return <p className="text-sm text-muted-foreground">Loading social publishing…</p>;
  ...
  return (
    <section className="mb-8 border border-border p-4 text-sm">
      ...
      <div className="mt-4 grid gap-6 lg:grid-cols-[minmax(0,280px)_1fr]">
        <div>
          {preview ? (
            ...
            <div className="flex aspect-[4/5] items-center justify-center border border-dashed border-border text-xs text-muted-foreground">
              No card yet
            </div>
          )}
        ...
        <div className="space-y-3">
          <label className="block text-xs">
            Card headline
            <input className="mt-1 w-full border border-border bg-background px-3 py-2" value={headline} onChange={(e) => setHeadline(e.target.value)} />
          </label>
          <label className="block text-xs">
            Supporting line
            <input className="mt-1 w-full border border-border bg-background px-3 py-2" value={support} onChange={(e) => setSupport(e.target.value)} />
          </label>
          <label className="block text-xs">
            Ratio
            <select className="mt-1 w-full border border-border bg-background px-3 py-2" value={ratio} onChange={(e) => setRatio(e.target.value as CardRatio)}>
              <option value="4:5">4:5</option>
              <option value="1:1">1:1</option>
            </select>
          </label>
          <div className="flex flex-wrap gap-2">
            <button type="button" disabled={busy} onClick={() => void generateCard(true)} className="bg-primary px-3 py-2 text-primary-foreground disabled:opacity-60">
              {social.data.cardImageUrl ? "Regenerate" : "Generate Photo Card"}
            </button>
            <button type="button" disabled={busy} onClick={() => void generateCard(false)} className="border border-border px-3 py-2 disabled:opacity-60">
              Preview
            </button>
            {preview ? (
              <a href={preview} download="the-connect-card.jpg" className="border border-border px-3 py-2">
                Save image
              </a>
            ) : null}
          </div>
          ...
          <label className="block text-xs">
            Facebook caption
            <textarea className="mt-1 min-h-28 w-full border border-border bg-background px-3 py-2" value={caption} onChange={(e) => setCaption(e.target.value)} />
          </label>
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              ...
              onClick={async () => {
                setBusy(true);
                try {
                  const out = await makeCaption({ data: { id: storyId } });
                  setCaption(out.caption);
                  toast.success(`Caption ready \u00b7 ${out.provider}`);
                  await refresh();
                } catch (err) {
            ...
            <button
              ...
              onClick={async () => {
                setBusy(true);
                try {
                  await persistCaption({ data: { id: storyId, caption } });
      ...
      <div className="mt-5 border border-border bg-secondary/40 p-3 text-xs">
        <p>
          <strong>Facebook:</strong> {fb.status}
          {fb.pageName ? ` \u00b7 ${fb.pageName}` : ""}
          {fb.pageIdMasked ? ` \u00b7 ${fb.pageIdMasked}` : ""}
        </p>
        ...
        <label className="mt-3 flex items-center gap-2">
          ...
          I reviewed the card, caption, URL and Page. Publish now.
        </label>
        <button
          type="button"
          disabled={busy || !confirmPublish || !fb.configured || fb.status === "published"}
          onClick={async () => {
            setBusy(true);
            try {
              const out = await publish({ data: { id: storyId, confirm: true } });
              toast.success(`Published to Facebook \u00b7 ${out.postId}`);
          ...
          className="mt-3 bg-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-60"
        >
          Publish to Facebook
        </button>
        {!fb.configured ? (
          <p className="mt-2 text-muted-foreground">Set META_ACCESS_TOKEN and META_PAGE_ID on the server, then open Desk Settings.</p>
        ) : null}
